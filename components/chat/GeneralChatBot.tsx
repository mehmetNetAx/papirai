...
export default function GeneralChatBot({ className }: GeneralChatBotProps) {
  const [messages, setMessages] = useState<Message[]>([]);
  const [input, setInput] = useState('');
  const [loading, setLoading] = useState(false);
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [relevantContracts, setRelevantContracts] = useState<RelevantContract[]>([]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.relevantContracts && data.relevantContracts.length > 0) {
        setRelevantContracts(data.relevantContracts);
      }
    } catch (err: any) {
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && (
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p>Tüm sözleşmeler hakkında sorular sorabilirsiniz.</p>
                <p className="text-sm mt-2">Örnek: "Hangi sözleşmeler yakında sona erecek?"</p>
              </div>
            )}
                ...
                <div className="space-y-1">
                  {relevantContracts.map((contract) => (
                    <Link
                      ...
                      className="block text-sm text-blue-700 dark:text-blue-400 hover:underline"
                    >
                      • {contract.title}
                    </Link>
                  ))}
